// $Id: clif.c,v 1.16 2004/01/21 04:55:57 rovert Exp $
...
int clif_skillinfoblock(struct map_session_data *sd)
{
	...
	for ( i = c = 0; i < MAX_SKILL; i++){
		if( (id=sd->status.skill[i].id)!=0 ){
			...
			WFIFOW(fd,len+10)= skill_get_range(id,sd->status.skill[i].lv);
...
int clif_skillup(struct map_session_data *sd,int skill_num)
{
	...
	WFIFOW(fd,8) = skill_get_range(skill_num,sd->status.skill[skill_num].lv);
...
int clif_item_skill(struct map_session_data *sd,int skillid,int skilllv,const char *name)
{
	...
	WFIFOW(fd,12)=skill_get_range(skillid,skilllv);
