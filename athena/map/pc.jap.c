...
static unsigned int equip_pos[11]={0x0100,0x0010,0x0020,0x0002,0x0004,0x0040,0x0008,0x0080,0x0200,0x0001,0x8000};
...
int pc_calcstatus(struct map_session_data* sd,int first)
{
	...
	for(i=0;i<10;i++) {
		...
		if(index < 0)
			continue;
		if(i == 3 && sd->equip_index[2] == index)
			continue;
		if(i == 8 && sd->equip_index[0] == index)
			continue;
		if(i == 9 && (sd->equip_index[0] == index || sd->equip_index[8] == index))
			continue;
			...
			if(sd->inventory_data[index]->type == 4) {
				...
				if(i == 2 && sd->status.inventory[index].equip == 0x20) {
					...
					if(sd->status.inventory[index].card[0]==0x00ff){	// 製造武器
						...
						sd->atk_ele_= (sd->status.inventory[index].card[1]&0x0f);	// 属 性
					}
					...
					if(sd->status.inventory[index].card[0]==0x00ff){	// 製造武器
						...
						sd->atk_ele = (sd->status.inventory[index].card[1]&0x0f);	// 属 性
					}
					sd->attackrange = sd->inventory_data[index]->range;
					run_script(sd->inventory_data[index]->equip_script,0,sd->bl.id,0);
				}
				...
				sd->watk += sd->inventory_data[index]->atk;
				sd->def += sd->status.inventory[index].refine*refinebonus[0][0];
				run_script(sd->inventory_data[index]->equip_script,0,sd->bl.id,0);
			}
		}
	}
...
//	sd->aspd += 10 * aspd_base[sd->status.class][weapontype] * (250 - sd->paramc[1] - sd->paramc[4]/4) /250;
	...
	if (pc_iscarton(sd) && (skill=pc_checkskill(sd,MC_PUSHCART))>0)	// カートによる速度低下
		sd->speed += (10-skill) * 0.1 * DEFAULT_WALK_SPEED;
	...
	sd->nhealhp = 1 + (sd->paramc[2]/6) + (sd->status.max_hp/200);
	...
	if(sd->sc_count){
		...
		if(sd->sc_data[SC_SIEGFRIED].timer!=-1){  // 不死身のジークフリード
			...
			sd->addele_[1] += sd->sc_data[SC_SIEGFRIED].val2;	// 水
			sd->addele[3] += sd->sc_data[SC_SIEGFRIED].val2;	// 火
			sd->addele_[3] += sd->sc_data[SC_SIEGFRIED].val2;	// 火
		}
		if(sd->sc_data[SC_PROVIDENCE].timer!=-1){	// プロヴィデンス
			sd->addele[6] += sd->sc_data[SC_PROVIDENCE].val2;	// 対 聖属性
			sd->addele_[6] += sd->sc_data[SC_PROVIDENCE].val2;	// 対 聖属性
			sd->addrace[6] += sd->sc_data[SC_PROVIDENCE].val2;	// 対 ?魔
			sd->addrace_[6] += sd->sc_data[SC_PROVIDENCE].val2;	// 対 ?魔
		}
...
int pc_bonus3(struct map_session_data *sd,int type,int type3,int val,int chance)	// RoVeRT
{
return 0;
}
