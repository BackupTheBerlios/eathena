// $Id: script.c,v 1.31 2004/02/26 00:50:33 sara-chan Exp $
//#define DEBUG_FUNCIN
//#define DEBUG_DISP
//#define DEBUG_RUN
	...
	for(p++;p && *p && *p!='}';){
		...
		if(*tmpp==':'){
			...
			*skip_word(p)=0;
			...
			strdb_insert(scriptlabel_db,p,script_pos);	// 外部用label db登?
			*skip_word(p)=c;
			p=tmpp+1;
			continue;
		}
