// $Id: char.c,v 1.4 2004/02/13 21:42:04 rovert Exp $
...
//}//---------------------------test count------------------------------	
	...
	for(i=0;i<10;i++){
		if(p->memo_point[i].map[0]){
			sprintf(tmp_sql,"INSERT delayed INTO `memo`(`char_id`,`map`,`x`,`y`) VALUES ('%d', '%s', '%d', '%d')",
	...
	for(i=0;i<p->global_reg_num;i++){
		if(p->global_reg[i].value !=0){
			sprintf(tmp_sql,"INSERT delayed INTO `global_reg_value` (`char_id`, `str`, `value`) VALUES ('%d', '%s','%d')",
...
int memitemdata_to_sql(struct itemtemp mapitem, int eqcount, int noteqcount, int char_id,int tableswitch){
	...
	if (sql_cres) {
		for(i=0;(sql_crow = mysql_fetch_row(sql_cres));i++){
			...
			else if (itemdb_isequip(atoi(sql_crow[1]))==0){				
			...
			if(!(dbitem.equip[j].flag==1)){
				if(mapitem.equip[i].nameid==dbitem.equip[j].nameid){
					...
					{	mapitem.equip[i].flag=1;
						...
						"`attribute`='%d', `card0`='%d', `card1`='%d', `card2`='%d', `card3`='%d' WHERE `%s`='%d' AND `nameid`='%d' AND `equip`='%d' AND `identify`='%d'" 
						"AND `refine`='%d' AND `attribute`='%d' AND `card0`='%d' AND `card1`='%d' AND `card2`='%d' AND `card3`='%d' LIMIT 1\n",
	...
	for(i=1;i<eqcount;i++){
		if(!(mapitem.equip[i].flag==1)){
			sprintf(tmp_sql,"INSERT delayed INTO `%s`(`id`,`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `card0`, `card1`, `card2`, `card3`)"
